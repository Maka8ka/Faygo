...
func AesCbcEncrypt(plainText, key []byte, ivAes ...byte) ([]byte, error) {
	...
	paddingText := PKCS5Padding(plainText, block.BlockSize())	var iv []byte
	if len(ivAes) != 0 {
		if len(ivAes) != 16 {
			return nil, nil 
		} else {
...
func AesCbcDecrypt(cipherText, key []byte, ivAes ...byte) ([]byte, error) {
	...
	blockMode.CryptBlocks(paddingText, cipherText)	plainText, err := PKCS5UnPadding(paddingText)
	if err != nil {
		return nil, err
	}
